...
import { filterProviderPickerOptions, prioritizeProviderSetupOptions, providerPickerPage, PROVIDER_PICKER_PAGE_SIZE, renderProviderSetupOptionLine } from "./provider-picker.js";
...
export class TuiApp {
  ...
  private async askModeObjective(label: string, defaultValue?: string): Promise<string> {
    ...
    try {
      const placeholder = label === "Loop objective" ? "Describe what Inferoa should keep working on" : label;
  ...
  private async chooseExternalProvider(options: ExternalProviderSetupOption[], currentProviderId?: string): Promise<ExternalProviderSetupOption> {
    ...
    let pageIndex = Math.floor(defaultIndex / PROVIDER_PICKER_PAGE_SIZE);
    let selected = defaultIndex % PROVIDER_PICKER_PAGE_SIZE;
    this.#rl?.pause();
  ...
  private async enqueueGoalContinuation(stateOrObjective: GoalState | string): Promise<void> {
    ...
    if (typeof stateOrObjective !== "string" && stateOrObjective.goal.preference === "replay") {
      ...
      if (!consumed) {
        ...
        const saved = writeGoalState(this.app.store, session.session_id, completeRepeatGoal(stateOrObjective, "Repeat loop finished."), runId);
        recordGoalCompletionReport(this.app.store, session.session_id, runId);
        this.renderGoalPanel(saved);
        return;
      }
...
function parseGoalRepeatRuns(input: string): number | undefined {
  const normalized = input.trim().toLowerCase();
  const match = /^(\d+)\s*(x|run|runs|loop|loops)?$/.exec(normalized);
  if (!match?.[1]) {
    return undefined;
  }
...
function buildDoctorToolsRegressionPrompt(): string {
  return [
    ...
    "- Cover read/search, file write/edit/patch, todo_write, code intelligence, process management, git read-only tools, resources/session notes, skills, and any configured network or Omni tools that are safe and already configured.",
    "- For hidden capabilities, explicitly use tool_search then capability_call, and include the capability_call target names as evidence in the report.",
