...
export function buildGoalWorkPrompt(goalOrObjective: GoalRecord | string): string {
  ...
  if (kind === "research") {
    return [
      ...
      "Continue the active research goal cycle.",
      "Use the goal horizon as the research cycle: keep goal steps current with goal op=update_step while maintaining research experiments with init_experiment, run_experiment, log_experiment, update_experiment, and update_notes.",
  ...
  return [
    ...
    "If this is Horizon 0 orientation, inspect enough context to infer the goal approach, call goal op=set_strategy with approach when needed, seed the candidate ledger with goal op=update_ledger, and complete the orientation steps.",
...
export function buildGoalReflectionPrompt(goalOrObjective: GoalRecord | string): string {
  ...
  if (kind === "research") {
    return [
      `Goal objective: ${objective}`,
      "Run an internal reflection for the active research goal.",
      "Step back from the current research cycle, experiment ledger, run history, benchmark evidence, guardrail evidence, notes, and goal plan.",
      "Treat the current experiments as hypotheses, not as the boundary of the research goal.",
