...
function optimizationSurfacesFigure(dataset) {
  ...
  const values = [
    { label: "Prefix cache prefill saved", value: pct(dataset.simulator.find((row) => row.label === "64 loops")?.prefill_work_saved_by_cache_tokens, dataset.simulator.find((row) => row.label === "64 loops")?.prompt_tokens), color: "#1976d2" },
    { label: "CodeGraph context saved", value: s.codegraph_context_savings_pct, color: "#00796b" },
    { label: "RTK command saved", value: s.rtk_savings_pct, color: "#8e24aa" },
    { label: "Router accuracy gain", value: router.deepseek_same_budget_accuracy_gain_pct, color: "#ef6c00" },
    { label: "Big-3 extra cost avoided", value: router.big3_extra_cost_pct, color: "#c62828" },
  ].filter((item) => typeof item.value === "number" && Number.isFinite(item.value));
  const w = 920;
  const h = 470;
  const p = { left: 86, right: 42, top: 60, bottom: 118 };
  const plotW = w - p.left - p.right;
  const plotH = h - p.top - p.bottom;
  const maxV = Math.max(100, ...values.map((item) => item.value));
  const barW = plotW / values.length - 28;
  return svg(w, h, `
    ${chartTitle("Inferoa value comes from several compounding surfaces", 28, 34)}
    ${axis(p, plotW, plotH, "Percent improvement")}
    ${gridY(p, plotW, plotH, [0, 25, 50, 75, 100], maxV)}
    ${values
      .map((item, i) => {
        const x = p.left + i * (plotW / values.length) + 16;
        const height = (plotH * item.value) / maxV;
        const y = p.top + plotH - height;
        return `<rect x="${x}" y="${y}" width="${barW}" height="${height}" rx="4" fill="${item.color}"/><text x="${x + barW / 2}" y="${y - 8}" text-anchor="middle" font-size="15" font-weight="700" fill="#263238">${item.value.toFixed(1)}%</text><text x="${x + barW / 2}" y="${h - 74}" text-anchor="middle" font-size="13" fill="#263238">${wrapSvgLabel(item.label, x + barW / 2, h - 74, 17)}</text>`;
      })
      .join("")}
    <text x="${p.left}" y="${h - 22}" font-size="13" fill="#607d8b">Sources: Inferoa Runtime stress suite, RTK command records, CodeGraph context projection, and routing benchmark projection.</text>
  `);
}
